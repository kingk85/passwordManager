...
//-------------------------------------------------------------------------------------------------------------  //
//                          XT-Crypter_V2.php By Kingk for Databases or Files Crypt                            		//
//-------------------------------------------------------------------------------------------------------------  //
// This script is a Cryptography tool, it can be used for Crypt Data bases or files without any external	//
// library, using a strong encryption algorithm developed by kingk.  		                                       	 		    //
// To use this script you have to include the file XT-Crypter_V2.php in each page that needs the Cryptography  	  	    //
// functions.                                                                                                					  		    //
//                                                                        	                            						       		   //
...
//                                                                                                             //
// X_DeCrypt_File($Input_Filename,$Output_filename, $Password, TRUE) //DecryptFile ( Return false if           //
// any problem )                                                                                               //
//                                                                                                             //
...
//                 Boolean value    ====> 'TRUE' for Base64 encode                                             //
//                                                                                                             //
//-------------------------------------------------------------------------------------------------------------//
//   For more info visit the official site : http://www.xploit-team.org or contact me kingk@xploit-team.org    //
//-------------------------------------------------------------------------------------------------------------//

define("MAX_PWL_LEN","65536");
define("BUFFER_SIZE","65536");

// FUNZIONE CHAR TO ASCII
...
// FUNZIONE CHAR TO ASCII
// FUNZIONE XOR \\
function cfr ( $inFILE, $pass, $Fsize, $lenPASS )
{
$passPOS = 0;
$passEND = $lenPASS -1;
$passREV = $lenPASS -1;
...
$passREV = $lenPASS -1;
if ( $passREV != $passPOS )
$inFILE{$j} = $inFILE{$j} ^ $pass{$passPOS} ^ $pass{ $passREV };
else
$inFILE{$j} = $inFILE{$j} ^ $pass{$passPOS};
$passPOS++;
$passREV--;
}
...
// Funzione ordinamento a bolla ( Bubblesort )
...
// Fine della funzione Bubblesort

// Funzione di ricerca lineare
function linearsearch( $array, $key, $size )
{
for( $n = 0; $n < $size; $n++) {
    if( $array[$n] == $key){
        $array[$n] = 300;
...
// Fine della funzione di ricerca lineare


function ordine ( $inFILE, $pass, $Fsize, $lenPASS, $ordpwl ) // funzione principale cifratrice
{
$ARR_TEMP = $inFILE;

	for ( $j = 0; $j < $Fsize - ( ( $Fsize ) % $lenPASS ); ) 
	{
		$tmpPOS = 0;
...
function ordinepwl ( $pass, $lenPASS ) // Ritorna un array di interi ordinato a seconda della password
{
	$passPOS = 0;
	$passEND = $lenPASS -1;
for ( $j = 0 ; $j < $lenPASS ; $j++ ) 
	{
	$NUM_ORDINE[$j]    = char_to_ascii ( $pass[$j] );
	$ARR_ORIGINALE[$j] = char_to_ascii ( $pass[$j] );
	}

	for ( $j = $lenPASS ; $j < 65536; $j++ ) //Azzera gli altri elementi del vettore?!
	{
	$NUM_ORDINE[$j] = 300;
	$ARR_ORIGINALE[$j] = 300;
	}
	$NUM_ORDINE = bubblesort( $NUM_ORDINE, $lenPASS );
	// RICERCALINEARE
	...
	for ( $j = 0 ; $j < $lenPASS ; $j++ )
	     {
	       $trovato = false;
               for( $n = 0; $n < $lenPASS; $n++ )
                             {
                           if( $ARR_ORIGINALE[$n] == $NUM_ORDINE[$j] and $trovato == false )
                               {
                                  $ARR_ORIGINALE[$n] = 300;
                                  $arrordine[$j] = $n;
                                  $trovato = true;
...
function deordine ( $inFILE , $pass, $Fsize, $lenPASS, $ordpwl ) // funzione per decifrare
{
for ( $j = 0; $j < $Fsize - ( ( $Fsize ) % $lenPASS )  ; )
{
...
function XtEnigmaKey ( $passWord, $RA, $RB, $RC, $RD, $RE, $RF, $RG, $RH, $RI, $RL, $RN, $RM, $RO, $RP, $RQ, $RW, $RR, $RT, $RY, $RU, $RgA, $RgB, $RgC, $RgD, $RgE, $RgF, $RgG, $RgH, $RgI, $RgL, $RgN, $RgM, $RgO, $RgP, $RgQ, $RgW, $RgR, $RgT, $RgY, $RgU, $IncA, $IncB, $IncC, $IncD, $IncE, $IncF, $IncG, $IncH, $IncI, $IncL, $IncN, $IncM, $IncO, $IncP, $IncQ, $IncW, $IncR, $IncT, $IncY, $IncU, $LenPwl )
{
...
for ( $i = 1; $i < $LenPwl; $i++ )
{
$ArTemp[ $count ] = char_to_ascii ( $passWord{$j} ^ $passWord{$i} );
$count++;
}
...
if ( $count < 19 )
{
	...
	{
	$ArTemp[$count] = char_to_ascii( $passWord{$i} );
	$count++;	
	}
}


// ARRAY TEMPORANEO CARICO..
...
{
	if ( char_to_ascii( $passWord{$i} ) %2 != 0 )
	$ArBool[ $count ] = true;
	else
	$ArBool[ $count ] = false;
	$count++;
...
if ( $LenPwl > 20 )
{
	...
	{
		if ( ( char_to_ascii ( $passWord{$j} ) + char_to_ascii ( $passWord{$i} ) ) > 255 )
		$ArTemp[ $count ] = ( ( char_to_ascii ( $passWord{$j} ) + char_to_ascii ( $passWord{$i} ) ) - 256 );
		else
		$ArTemp[ $count ] = char_to_ascii ( $passWord{$j} ) + char_to_ascii ( $passWord{$i} );
		$count++;
		$j++;
	}
...
$ChrTemp =  char_to_ascii( $inFILE{$i} );
...
//////////////////////////////////////////// CILINDRO B \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
if ( $Riporto[0] == true )  // Se c'� riporto..
{
...
$Riporto[1] = false; // Non c'� nessun riporto
}
...
if ( $Riporto[1] == true ) // Se � presente il riporto[1]..
{
...
$Riporto[3] = false; // Non c'� nessun riporto
}
...
$Riporto[5] = false; // Non c'� nessun riporto
}
...
$Riporto[6] = false; // Non c'� nessun riporto
}
...
$Riporto[7] = false; // Non c'� nessun riporto
}
...
$Riporto[8] = false; // Non c'� nessun riporto
}
...
$Riporto[9] = false; // Non c'� nessun riporto
}
...
$Riporto[10] = false; // Non c'� nessun riporto
}
...
$Riporto[11] = false; // Non c'� nessun riporto
}
...
$Riporto[12] = false; // Non c'� nessun riporto
}
...
$Riporto[13] = false; // Non c'� nessun riporto
}
...
$Riporto[14] = false; // Non c'� nessun riporto
}
...
$Riporto[15] = false; // Non c'� nessun riporto
}
...
$Riporto[18] = false; // Non c'� nessun riporto
}
}
...
$Riporto[19] = false; // Non c'� nessun Riporto
}
...
$ChrTemp = $ChrTemp - $RA;
//cout<<"\n Sott$RAzione CharTemp - $RA risultato :"<<$ChrTemp;
$ChrTemp =  $RotoreA[$ChrTemp];
//cout<<"\n Valore nell AR$RAY A di "<<$ChrTemp;
// STEP 1

// STEP 2
//cout<<"\n Sott$RAzione CharTemp : "<<$ChrTemp<< " - RB : "<<RB<<" risultato :"<<$ChrTemp;
if ( ( $ChrTemp - $RB ) < 0 )
$ChrTemp = ( ( $ChrTemp - $RB) + 256 );
else
$ChrTemp = $ChrTemp - $RB;
...
$ChrTemp =  $RotoreB[$ChrTemp];
//cout<<"\n Valore nell ARRAY B di "<<$ChrTemp;
// STEP 2
...
$ChrTemp =  $RotoreC[$ChrTemp];
//cout<<"\n Valore nell ARRAY C di "<<$ChrTemp;
// STEP 3
...
$ChrTemp =  $RotoreD[$ChrTemp];
//cout<<"\n Valore nell ARRAY D di "<<$ChrTemp;
// STEP 4
...
$ChrTemp =  $RotoreE[$ChrTemp];
//cout<<"\n Valore nell ARRAY E di "<<$ChrTemp;
// STEP 5
...
$ChrTemp =  $RotoreF[$ChrTemp];
//cout<<"\n Valore nell ARRAY F di "<<$ChrTemp;
// STEP 6
...
$ChrTemp =  $RotoreG[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY G di "<<$ChrTemp;
// STEP 7
...
$ChrTemp =  $RotoreH[$ChrTemp];
//cout<<"\n Valore nell ARRAY H di "<<$ChrTemp;
// STEP 8
...
$ChrTemp =  $RotoreI[$ChrTemp];
//cout<<"\n Valore nell ARRAY I di "<<$ChrTemp;
// STEP 9
...
$ChrTemp =  $RotoreL[$ChrTemp];
//cout<<"\n Valore nell ARRAY L di "<<$ChrTemp;
// STEP 10
...
$ChrTemp =  $RotoreN[$ChrTemp];
//cout<<"\n Valore nell ARRAY N di "<<$ChrTemp;
// STEP 11
...
$ChrTemp =  $RotoreM[$ChrTemp];
//cout<<"\n Valore nell ARRAY M di "<<$ChrTemp;
// STEP 12
...
$ChrTemp =  $RotoreO[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY O di "<<$ChrTemp;
// STEP 13
...
$ChrTemp =  $RotoreP[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY P di "<<$ChrTemp;
// STEP 14
...
$ChrTemp =  $RotoreW[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY W di "<<$ChrTemp;
// STEP 16
...
$ChrTemp =  $RotoreR[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY R di "<<$ChrTemp;
// STEP 17
...
$ChrTemp =  $RotoreT[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY T di "<<$ChrTemp;
// STEP 18
...
$ChrTemp =  $RotoreY[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY Y di "<<$ChrTemp;
// STEP 19
...
$ChrTemp =  $RotoreU[ $ChrTemp ];
//cout<<"\n Valore nell ARRAY U di "<<$ChrTemp;
// STEP 20
...
function X_Crypt_String($Stringa, $Password, $Base64 )
{
$VrA = 0; $VrB = 0; $VrC = 0; $VrD = 0; $VrE = 0; $VrF = 0; $VrG = 0; $VrH = 0; $VrI = 0; $VrL = 0; $VrN = 0; $VrM = 0; $VrO = 0; $VrP = 0; $VrQ = 0; $VrW = 0; $VrR = 0; $VrT = 0; $VrY = 0; $VrU = 0; $incA = 1; $incB = 1; $incC = 1; $incD = 1; $incE = 1; $incF = 1; $incG = 1; $incH = 1; $incI = 1; $incL = 1; $incN = 1; $incM = 1; $incO = 1; $incP = 1; $incQ = 1; $incW = 1; $incR = 1; $incT = 1; $incY = 1; $incU = 1; //Parametri primari dell Enigma
$RdA = false; $RdB = false; $RdC = false; $RdD = false; $RdE = false; $RdF = false; $RdG = false; $RdH = false; $RdI = false; $RdL = false; $RdN = false; $RdM = false; $RdO = false; $RdP = false; $RdQ = false; $RdW = false; $RdR = false; $RdT = false; $RdY = false; $RdU = false;

XtEnigmaKey ( $Password,  $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, strlen($Password) );
$ArrayOrdinePwl = ordinepwl( $Password, strlen($Password) ); // Array con l'ordine della password
$Stringa =  XtEnigma ( $Stringa, strlen( $Stringa ), $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU );
$Stringa = cfr ( $Stringa, $Password, strlen($Stringa), strlen($Password) );
$Stringa = ordine ( $Stringa, $Password, strlen($Stringa), strlen($Password), $ArrayOrdinePwl );
if ( $Base64 == TRUE )
$Stringa = base64_encode ( $Stringa );
return ($Stringa);
}

function X_DeCrypt_String($Stringa, $Password, $Base64 )
{
$VrA = 0; $VrB = 0; $VrC = 0; $VrD = 0; $VrE = 0; $VrF = 0; $VrG = 0; $VrH = 0; $VrI = 0; $VrL = 0; $VrN = 0; $VrM = 0; $VrO = 0; $VrP = 0; $VrQ = 0; $VrW = 0; $VrR = 0; $VrT = 0; $VrY = 0; $VrU = 0; $incA = 1; $incB = 1; $incC = 1; $incD = 1; $incE = 1; $incF = 1; $incG = 1; $incH = 1; $incI = 1; $incL = 1; $incN = 1; $incM = 1; $incO = 1; $incP = 1; $incQ = 1; $incW = 1; $incR = 1; $incT = 1; $incY = 1; $incU = 1; //Parametri primari dell Enigma
$RdA = false; $RdB = false; $RdC = false; $RdD = false; $RdE = false; $RdF = false; $RdG = false; $RdH = false; $RdI = false; $RdL = false; $RdN = false; $RdM = false; $RdO = false; $RdP = false; $RdQ = false; $RdW = false; $RdR = false; $RdT = false; $RdY = false; $RdU = false;

XtEnigmaKey ( $Password,  $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, strlen($Password) );
$ArrayOrdinePwl = ordinepwl( $Password, strlen($Password) );
if ( $Base64 == TRUE )
$Stringa = base64_decode ( $Stringa );

$Stringa = deordine ( $Stringa, $Password, strlen($Stringa), strlen($Password), $ArrayOrdinePwl );
$Stringa = cfr ( $Stringa, $Password, strlen($Stringa), strlen($Password) );
$Stringa =  XtEnigmaD ( $Stringa, strlen( $Stringa ), $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU );
...
function X_Crypt_File($IN_filename,$OUT_filename, $Password, $Base64)
{
if (file_exists("$IN_filename"))
{
$CorrectBuffer = 65536 - ( 65536 % strlen($Password) );
$handle  = fopen($IN_filename, "r");
$handle2 = fopen($OUT_filename,'w'); //apertura per scrittura
...
while (!feof($handle)) 
{
	$buffer = fread($handle, $CorrectBuffer);

	$buffer =  XtEnigma ( $buffer, strlen( $buffer ), $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU );
	$buffer = cfr ( $buffer, $Password, strlen($buffer), strlen($Password) );
	$buffer = ordine ( $buffer, $Password, strlen($buffer), strlen($Password), $ArrayOrdinePwl );
	if ( $Base64 == TRUE )
	$buffer = base64_encode ( $buffer );
	fwrite($handle2,$buffer); //scrittura
}
fclose($handle);
fclose($handle2);
return (TRUE);
...
function X_DeCrypt_File($IN_filename,$OUT_filename, $Password, $Base64)
{
if (file_exists("$IN_filename"))
{
$CorrectBuffer = 65536 - ( 65536 % strlen($Password) );
...
$buffer = deordine ( $buffer, $Password, strlen($buffer), strlen($Password), $ArrayOrdinePwl );
$buffer = cfr ( $buffer, $Password, strlen($buffer), strlen($Password) );
$buffer =  XtEnigmaD ( $buffer, strlen( $buffer ), $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU, $RdA, $RdB, $RdC, $RdD, $RdE, $RdF, $RdG, $RdH, $RdI, $RdL, $RdN, $RdM, $RdO, $RdP, $RdQ, $RdW, $RdR, $RdT, $RdY, $RdU, $incA, $incB, $incC, $incD, $incE, $incF, $incG, $incH, $incI, $incL, $incN, $incM, $incO, $incP, $incQ, $incW, $incR, $incT, $incY, $incU, $VrA, $VrB, $VrC, $VrD, $VrE, $VrF, $VrG, $VrH, $VrI, $VrL, $VrN, $VrM, $VrO, $VrP, $VrQ, $VrW, $VrR, $VrT, $VrY, $VrU );
fwrite($handle2,$buffer);      //scrittura
}
fclose($handle);
fclose($handle2);
...
//echo X_Crypt_String("stringa di prova", "pass", TRUE);
//echo X_Decrypt_String(X_Crypt_String("stringa di prova", "pass", TRUE), "pass", TRUE);
